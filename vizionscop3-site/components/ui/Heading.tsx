...
type LevelProp = HeadingLevel | "1" | "2" | "3" | "4" | "5" | "6";

interface HeadingProps extends React.HTMLAttributes<HTMLHeadingElement> {
  /** Semantic heading level */
  as?: LevelProp;
  /** Alias for `as` (legacy call sites); also accepts "1".."6" */
  level?: LevelProp;
...
function resolveHeadingTag(prop?: LevelProp): HeadingLevel {
  if (!prop) return "h2";
  const s = String(prop);
  if (/^[1-6]$/.test(s)) return `h${s}` as HeadingLevel;
  return prop as HeadingLevel;
}

const Heading = React.forwardRef<HTMLHeadingElement, HeadingProps>(
  ({ className, as, level, size = "lg", children, ...props }, ref) => {
    const Component = resolveHeadingTag(as ?? level);
    return (
      <Component
        ref={ref}
        className={cn(
